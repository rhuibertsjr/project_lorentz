...
#define INTERNAL_STATEMENT(S) do { S } while(0)
#define INTERNAL_UNUSED(x) (void)(x) 
...
# define log(tag, type, message, ...)                                          \
  INTERNAL_STATEMENT(_internal_stdout_log(tag, type, message, ##__VA_ARGS__);)
#else
# define log(tag, type, message, ...)
#endif
...
# define assert(condition, ...)                                                \
  INTERNAL_STATEMENT( if(!(condition)) {                                       \
